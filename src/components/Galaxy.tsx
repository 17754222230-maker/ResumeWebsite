...
/**
 * React Bits Galaxy（ogl WebGL 星空）TypeScript 适配版。
 * 与原版差异：
 * 1) 鼠标 mousemove 监听挂在 window 上（原版挂容器）——背景层可 pointer-events-none，
 *    既不遮挡内容点击，又保留鼠标斥力互动；mouseleave 挂 documentElement。
 * 2) prefers-reduced-motion 时直接不渲染（省 GPU）。
 * 3) props 类型化；容器样式内联（等价原 Galaxy.css 的 100% 宽高）。
 */
...
export default function Galaxy({
  ...
  useEffect(() => {
    ...
    const focalArr = focalKey.split(",").map(Number);
    const rotationArr = rotationKey.split(",").map(Number);
    ...
    // 鼠标监听挂 window：容器可 pointer-events-none，互动与点击穿透兼得；
    // 始终挂载、用 ref 门控，便于 mouseInteraction 切换时免重建上下文
  ...
  // saturation / mouseRepulsion / mouseInteraction 热更新（不重建 WebGL 上下文）
  useEffect(() => {
