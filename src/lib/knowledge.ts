...
export const profile: Profile = {
  ...
  title: "AI 全栈开发工程师",
  slogan:
    "从 7×24 产线到 LLM Agent，做同一件事：让系统给出的答案可验证、可追溯。",
  summary:
    "Java 后端出身，路径是 MES → ERP → 在线教育 → 飞猪机票 → LLM Agent：从物理约束极强的产线系统，一路走到输出本身带概率的 AI 系统。在制造业四年练出的幂等、对账、归因、兜底这套确定性工程纪律，恰好是 AI 应用最缺的部分——现在做的是把两者接起来：Spring AI / MCP / 双通道 RAG 负责能力上限，规则校验、引擎验价、人工终审与降级兜底负责守住风险下限。技术栈覆盖 Java、Python、C++、TypeScript。",
  contact: {
...
export const projects: Project[] = [
  ...
  },
  {
    ...
    highlights: [
      ...
      "基于 Spring AI Alibaba 搭建航司政策文件解析链路，Prompt 按 @Switch 版本化可灰度回滚，守住‘投放生效前必须人工确认’的红线",
      "Flowable 任务清单编排 SOP：上传→AI 解析→预校验→引擎验价→抽检→投放，失败处理下沉到应用层线程池（core4/max8/queue50 + AbortPolicy）与阶段状态枚举，半成品政策不流到线上",
    ],
    period: "2026.04 - 至今",
    company: "飞猪（阿里巴巴旗下）",
    metrics: [
    ...
    responsibilities: [
      ...
      "用 Flowable 任务清单把上传→AI 解析→预校验→引擎验价→抽检→投放串成 SOP，失败处理落在应用层线程池与阶段状态枚举上，不让半成品政策流到线上",
      "做验价链路的可观测性：用 EagleEye 把主线程、异步线程和 MQ 回调的 traceId 统一起来，在抽检/校验域再加 bizTraceId 把一次政策核验的相关日志串成一条线",
      "开发数据看板：Hologres 实时查询、EasyExcel 异步导出、OSS 大字段（fieldInfoList）卸载与下载全链路，处理过 DWD 层数据断档的治理和补数",
      ...
      "实现异步工作流与配置驱动：POST /api/workflows/start 秒级返回 taskId 后台执行全流程（模板刷新→工作区创建→OSS 下载→知识注入→Agent 执行→OSS 上传），MongoDB 持久化 Prompt 与任务状态、Redis 承载运行事件流（MAX_EVENTS 200 / TTL 7 天）；幂等锁防重复提交，超 30min 无进度的僵尸任务读侧自愈置失败；Diamond 配置中心实现航司灰度白名单与模型动态路由（精确匹配→通配→兜底三级降级），热更新订阅",
    ...
    fullTechStack: ["JDK 17", "Spring Boot 3.2", "Spring AI Alibaba (DashScope)", "Flowable 6.8", "Next.js 15", "Vercel AI SDK", "MCP (@modelcontextprotocol/sdk)", "MongoDB", "ioredis", "Python（Agent 运行时执行）", "HSF", "MetaQ/RocketMQ", "TDDL", "Tair", "Diamond", "SchedulerX", "EagleEye", "MySQL", "Hologres", "OSS", "EasyExcel"],
  },
  // ===== 个人项目 =====
  {
    slug: "bp-agent",
    ...
    category: "personal",
    description:
      "面向职业/半职业战队的电竞赛事 BP 实时决策与模拟训练系统。针对 BO5 全局单边 BP 赛制下决策空间指数级收缩、30 秒操作时限内教练认知负荷过高的问题，用 LangGraph 编排『规则引擎 + LLM 双引擎 + 三级降级』的决策链路：常规局面由确定性规则秒级给解，复杂博弈才升到大模型，延迟预算耗尽时逐级降档兜底，保证在时限内始终给出合法、可解释、可回测的针对性 BP 建议。",
    tags: ["Python", "FastAPI", "LangGraph", "通义千问 DashScope", "RAG", "决策系统"],
    role: "全栈开发",
    highlights: [
      "规则引擎 + LLM 双引擎 + 三级降级：常规局面确定性规则秒级给解，复杂博弈升 qwen-plus，超时逐级降到 qwen-flash 与纯规则兜底，LLM 调用外包熔断器",
      "延迟预算驱动实时性：全局 deadline 注入，各节点按剩余预算动态收缩超时并为降级链预留缓冲，30 秒 BP 时限内决策链路零超时",
      "全离线兜底：拔 API Key、断网、无 Redis 仍能降级到确定性规则给出合法建议",
      "结构化可解释输出：JSON Schema + Pydantic 校验 + 失败重试，合法性闸门强制大模型输出过 BP 规则，每条推荐附 Counter / 协同 / 熟练度 / 全局保留价值四维理由",
      "量化回测验证：285 场真实 BO5 赛事线性化回放算 Top-1/Top-K 命中率，按时间 holdout 划分防数据泄漏",
    ],
    period: "2026.05 - 至今",
    company: "个人项目",
    metrics: [
    ...
    responsibilities: [
      ...
      "构建自动化赛事数据爬虫与清洗管线：抓取官网 API 的 490 场真实 BO5 赛果，归一化入历史库，作为量化回测与对手 Ban/Pick 偏好分析的数据底座",
      "设计结构化可解释输出：response_format=json_object + Pydantic 强校验 + 失败重试，四维理由（Counter 匹配度 / 阵容协同 / 选手熟练度 / 全局保留价值）；输出层加合法性闸门强制大模型结果通过 BP 合法性规则，非法建议拦截重生成",
      "实现 16 步 BP 状态机与 BO5 跨局单边记忆：越序操作 422 拦截，全局单边 Ban/Pick 状态跨小局延续",
      "搭建知识管理后台：英雄克制/协同、Tier List 等知识经 Pydantic 校验 + 交叉引用校验 + 原子写入，支持时间戳快照备份 / 一键回滚与知识包内容指纹（sha1）版本标记，更新后热重载并异步重建向量库",
      "编写量化回测脚本：285 场 BO5 线性化回放计算 Top-1/Top-K 命中率，按时间 holdout 切分评估集防止用未来数据评估过去，并显式声明回放的顺序假设",
    ],
    challenges: [
      {
        challenge: "30 秒 BP 操作时限 vs 大模型 2-8s 推理延迟，直接调 LLM 无法保障实时性与稳定性",
        solution: "用 LangGraph 编排规则引擎 + LLM 双引擎链路，常规局面由确定性规则秒级给解，仅复杂博弈升 qwen-plus；配合全局延迟预算、节点级超时收缩、三级降档（plus→flash→纯规则）与熔断器，决策链路在时限内零超时，极端情况（断网/无缓存）也能降级给出合法建议",
      },
      {
        challenge: "大模型可能输出当前不可选/已被 ban 的英雄，或给出不符合 BP 规则的建议，直接采用会出事故",
        solution: "在输出层加合法性闸门，强制 LLM 结果过一遍 BP 合法性规则（已 ban/已选/阵营约束），非法建议拦截并重生成；配合 JSON Schema + Pydantic 校验与失败重试，保证落地建议始终合法且结构化",
      },
      {
        challenge: "AI 决策系统最怕『看起来很准但没法验证』，需要客观衡量推荐质量、避免自欺",
        solution: "用 285 场真实 BO5 赛事线性化回放做量化回测，计算 Top-1/Top-K 命中率；按时间 holdout 划分评估集避免用未来数据评估过去，并显式声明回放的顺序假设，让效果可复现、可追问",
      },
    ],
    achievements: [
      "决策链路在 30 秒 BP 时限内零超时，三级降级 + 熔断保证超时/断网/无缓存等极端情况仍给出合法建议",
      "每条推荐均附四维可解释理由并强制过合法性闸门，输出结构化、可复核",
      "基于 490 场真实赛事数据构建历史库与回测集，285 场 BO5 回放量化 Top-K 命中率",
      "21 个测试文件、319 个测试用例覆盖规则引擎、状态机、降级链、图流程与知识管理",
    ],
    fullTechStack: ["Python 3.11", "FastAPI", "LangGraph", "通义千问 DashScope (qwen-flash/plus/max)", "chromadb", "Redis", "Pydantic v2", "WebSocket", "React 18 + Vite"],
  },
  {
    slug: "resume-website",
    title: "个人简历与智能知识站",
...
export const experiences: Experience[] = [
  ...
  },
  {
    ...
    period: "2022.07 - 2026.01",
    subtitle:
      "从 7×24 产线 MES 到在线交易平台：四年项目制交付，把幂等、对账、归因、兜底练成肌肉记忆，也养成先领域建模、再写代码的工程习惯",
    description:
