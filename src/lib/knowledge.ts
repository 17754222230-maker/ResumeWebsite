...
export const projects: Project[] = [
  // ===== 飞猪项目（2026.04 - 至今）=====
  {
    ...
    title: "飞猪机票代理商经营项目",
    description:
      "飞猪机票代理商的经营平台，属于机票后端的核心域。我负责验座、验价、询价、收益计算、兜底出票这几个环节的开发，系统日均处理数万笔机票交易。",
    tags: ["Java", "SpringBoot", "SpringCloudAlibaba", "RocketMQ", "Redis", "高并发"],
    role: "后端开发工程师",
    highlights: [
      "负责验座、验价、询价、收益计算、兜底出票链路的日常开发与迭代",
      "机票搜索和价格计算并发高、对实时性敏感，做过几轮缓存与异步化优化",
      "ToC 系统面向千万级用户，出问题直接影响出票，稳定性是第一优先级",
    ],
    period: "2026.04 - 至今",
    company: "飞猪（阿里巴巴旗下）",
    responsibilities: [
      "开发机票核心域后端功能：验座、验价、询价、收益计算、兜底出票",
      "对接多家航司和代理商接口，处理库存与价格的实时同步",
      "针对机票搜索、价格计算这类高并发链路做性能优化",
      "参与 ToC 端线上问题排查，值班处理出票链路的异常单据",
    ],
    challenges: [
      {
        challenge: "机票价格实时性要求高，一次查询要聚合多个数据源，接口响应偏慢",
        solution: "给核心查询加 Redis 缓存并设计预热与失效策略，部分链路改成异步，用 RocketMQ 削峰。价格这类强实时数据只缓存短 TTL，宁可多回源也不给用户看过期价",
      },
      {
        challenge: "兜底出票要跨多个系统协调，中间任何一步失败都可能出现状态不一致",
        solution: "用 Seata 做最终一致性，再加本地消息表和定时补偿兜底，避免出重复票或漏单",
      },
    ],
    achievements: [
      "日均数万笔机票交易平稳处理",
      "核心查询链路经过缓存与异步化改造，响应时间明显下降",
    ],
  },
  {
    ...
    title: "商家运营后台经营提效项目",
    description:
      "商家运营后台的 AI 提效项目。航司政策文件（PDF/Word）以前靠运营人工阅读、逐条录成飞猪内部的政策格式，这个项目用大模型自动做规则提取和格式转换，解析引擎基于 Spring AI 搭建。",
    tags: ["Java", "Spring AI", "AI Coding", "大模型", "智能文档解析", "政策转换"],
    role: "后端开发工程师 / AI 集成",
    highlights: [
      "基于 Spring AI 搭建航司政策文件的解析与转换引擎",
      "大模型提取政策规则后转成飞猪内部统一政策格式，解析不了的挂起走人工",
      "运营不用再逐条手工录入政策，只做校验和兜底",
    ],
    period: "2026.04 - 至今",
    company: "飞猪（阿里巴巴旗下）",
    responsibilities: [
      "负责 AI 解析引擎的设计与开发，基于 Spring AI 搭建主流程",
      "对接大模型 API，反复调提示词，让政策规则的提取结果稳定、可结构化落库",
      "开发政策文件预处理管线，处理 PDF、Word 等不同来源格式",
      "和运营一起过解析出错的 case，靠人工校验反馈迭代准确率",
    ],
    challenges: [
      {
        challenge: "航司政策文件格式五花八门（PDF/Word/扫描件），非结构化文本没法直接喂给模型",
        solution: "拆成多步管线：先做 OCR 和文本提取，再让大模型分段理解，最后规则引擎校验输出格式，校验不过的挂起转人工，不硬猜",
      },
      {
        challenge: "政策里的行业术语和隐含规则，模型经常理解偏",
        solution: "整理行业术语词库，在提示词里配少样本示例，再靠运营人工校验的反馈持续修正提取规则",
      },
    ],
    achievements: [
      "常见格式的政策文件基本能自动转换，人工只做校验和兜底",
      "运营处理一份政策文件的时间明显缩短，不再逐条手工录入",
    ],
...
export const experiences: Experience[] = [
  ...
  },
  {
    ...
    period: "2022.07 - 2026.01",
    description:
      "先后参与马钢 MES、三钢 ERP、在线教育平台、中铝 ERP 四个项目的后端开发，从制造业现场系统做到互联网交易链路。",
    highlights: [
